...
//   The broken (computed) private key for the above is 4105243553
  ...
  // Use Fermat's factorization method to calculate q and p.
  mpz_class a;
  mpz_class a_squared;
  mpz_class difference; // Intermediate parameters in Fermat's method
  
  mpz_sqrt(a.get_mpz_t() ,(rsa.n).get_mpz_t()); 

  do{
    a = a + 1;
    mpz_pow_ui(a_squared.get_mpz_t(), a.get_mpz_t(), 2);
    difference = a_squared - rsa.n;
  }while( !(mpz_perfect_power_p(difference.get_mpz_t())) );

  mpz_class b;
  mpz_sqrt(b.get_mpz_t() ,difference.get_mpz_t());
  
  p_broken = a + b;
  q_broken = a - b;
  // Set rsa.d to the calculated private key d
  // rsa.d = mpz_class(// broken d value here) 
  phi = (p_broken-1) * (q_broken-1);
